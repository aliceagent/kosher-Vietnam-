...
export function ShabbatMap({
  ...
  const visible = showModeToggle && mode === "shabbat" ? pins.filter((pin) => pin.shabbatOk) : pins;
    ...
    async function draw() {
      ...
      for (const pin of visible) {
        const icon = L.divIcon({
          className: "",
          html: `<div style="width:14px;height:14px;border-radius:99px;background:${colors[pin.kind] ?? "#2a3f38"};border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,.35)"></div>`,
          iconSize: [14, 14],
          iconAnchor: [7, 7],
        });
      ...
      {showModeToggle ? (
        <>
          ...
          <p className="mt-3 text-xs font-medium text-stone">
            Walking minutes assume 4.5 km/h from the Chabad pin. Distances are not a hetter. Confirm the address
            before Shabbat.
          </p>
        </>
      ) : null}
